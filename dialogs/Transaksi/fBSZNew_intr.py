class fBSZNew:
  ...
  def CreateNewBSZ(self, DonorId, BSZId=0, restriction=0):
    ...
    self.FormObject.SetDataWithParameters(self.app.CreateValues(['DonorId',DonorId]))
  ...
  def bSaveClick(self,sender) :
    ...
    form.CommitBuffer()
    self.CheckInput()
    resp = form.CallServerMethod('BSZNew',form.GetDataPacket())
    
    status = resp.FirstRecord
    if status.IsErr : raise '',status.ErrMessage
    
    self.uipData.Edit()
    self.uipData.BSZId = status.BSZId
    sender.ExitAction = 1
