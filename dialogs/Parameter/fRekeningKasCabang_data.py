...
def OnSetData(sender):
  ...
  BranchCash = helper.GetObjectByInstance('BranchCash', sender.ActiveInstance)

  rec.SetFieldByName('LBranch.Kode_Cabang',BranchCash.BranchCode)

  rec.SetFieldByName('LCurrency.Currency_Code',BranchCash.CurrencyCode)

  corporate = helper.CreateObject('Corporate')
  CabangInfo = corporate.GetCabangInfo(BranchCash.BranchCode)
...
def SimpanData(config, parameter, returnpacket) :
    ...
    #Parameter
    ObjName = 'BranchCash'
    LsFieldInput = ('AccountName','Status','BranchCode','CashCode',
    ...
    try :
        ...
        if rsrc.mode == 'New' :
          rsrc.AccountNoId = 'BC.%s.%s' % (rsrc.BranchCode,rsrc.CurrencyCode)
