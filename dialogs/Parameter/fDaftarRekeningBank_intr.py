class fDaftarRekeningBank:
  ...
  def bExportExcelClick(self,sender):
    ...
    workbook = self.oPrint.OpenExcelTemplate(app,'tplMasterRekeningBank.xls')
    ...
    try:
      ...
      while i < ds.RecordCount:
        ...
        workbook.SetCellValue(row, 9, rec.BankAccountNo)
        workbook.SetCellValue(row, 10, rec.BankName)
        workbook.SetCellValue(row, 11, rec.Status)
