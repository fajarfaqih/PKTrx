class fProgram:
  ...
  def bOKClick(self, sender):
    if self.pAction_bClose.Visible :
      ...
      formid = 'parameter/fDaftarProgram'
      qForm = self.app.FindForm(formid)
      qPanel = qForm.FormObject.GetPanelByName('Program')
      qPanel.Refresh()

    #if self.mode in ['NewGroup','NewDetail'] :
    #    self.app.ShowMessage('Data program baru telah berhasil disimpan')
  ...
  def DetailOnClick(self,sender):
    ...
    Fields = ['PercentageOfAmilFunds',
              'FixedValue','FixedValueAmount','FundCategory','MultiPackage']
    uipData = self.uipData
    uipData.Edit()
