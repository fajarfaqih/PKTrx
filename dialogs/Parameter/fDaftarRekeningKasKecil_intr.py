class fDaftarRekeningKasKecil:
  ...
  def bExportExcelClick(self,sender):
    ...
    workbook = self.oPrint.OpenExcelTemplate(app,'tplMasterRekeningKasKecil.xls')
    ...
    try:
      ...
      while i < ds.RecordCount:
        ...
        workbook.SetCellValue(row, 4, rec.UserName)
        workbook.SetCellValue(row, 5, rec.BranchCode)
        workbook.SetCellValue(row, 6, rec.BranchName)
        workbook.SetCellValue(row, 7, rec.CurrencyCode)
        workbook.SetCellValue(row, 8, rec.CurrencyName)
        workbook.SetCellValue(row, 9, rec.Balance)
        workbook.SetCellValue(row, 10, rec.Status)
