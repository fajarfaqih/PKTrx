...
def DoProcess(config,App, Params) :
  ...
  strDeprDate = (config.FormatDateTime('YYYY-MM-DD', ProcessDate))
  config.SendDebugMsg(strDeprDate)

  sSQL = " select accountno from depreciableasset where deprstate='A' and \
  ...
  while not res.Eof:
    ...
    while oAccount.GetAsTDateTime('TanggalProsesBerikut') <= ProcessDate :
      intDeprDate = oAccount.GetAsTDateTime('TanggalProsesBerikut')
      ...
      #-- try..except
      config.FlushUpdates()
    #-- end while oAccount.GetAsTDateTime('TanggalProsesBerikut') <= ProcessDate
