...
def BranchDistribution(helper,oTran,oBatch,request,params):
  ...
  oSCashAccount = helper.GetObject('CashAccount',
       str(request['SourceAccountNo'])).CastToLowestDescendant()
  ...
  oDCashAccount = helper.GetObject('BranchCash',
       str(request['DestAccountNo'])).CastToLowestDescendant()
...
def BranchDistributionReturn(config, srequest, params):
  request = simplejson.loads(srequest)
  helper = phelper.PObjectHelper(config)
  ...
  try:
    ...
    if request[u'Amount'] > 0.0 :
      ...
      oDBranchCash = helper.GetObjectByNames('BranchCash',
        { 'BranchCode'   : request[u'BranchCodeDestination'],
          'CurrencyCode' : '000'
        }).CastToLowestDescendant()
      if oDBranchCash.isnull:
        raise 'Cash/Bank', 'Kas Cabang tidak ditemukan'
      
      oItemDCA = oTran.CreateAccountTransactionItem(oDBranchCash)
      oItemDCA.SetMutation('D', request[u'Amount'], 1.0)    
      oItemDCA.Description = request[u'Description']
      oItemDCA.SetJournalParameter('15')
...
#         oItemBudget.CreateBudgetTransaction(oBudget.BudgetId)
      ...
      oDistInfo = helper.GetObject('DistributionTransferInfo', oRefTransItem.TransactionId)
